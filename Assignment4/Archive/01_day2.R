...
# Note 3. Phylogeny of the sevent ant species:
...
colnames(ortholog_exp.norm) = colnames(ortholog_exp)
rownames(ortholog_exp.norm) = rownames(ortholog_exp)
...
ggplot(pca.data, aes(x = Dim.1, y = Dim.2, color = species, shape = caste)) +
  geom_point(size=3) +
  coord_fixed()
